...
FPLRichTextSyntaxHighlighterTextLayoutMarshaller::FPLRichTextSyntaxHighlighterTextLayoutMarshaller(TSharedPtr< FPrologSyntaxTokenizer > InTokenizer, const FSyntaxTextStyle& InSyntaxTextStyle)
...
FPLRichTextSyntaxHighlighterTextLayoutMarshaller::~FPLRichTextSyntaxHighlighterTextLayoutMarshaller()
{
}

void FPLRichTextSyntaxHighlighterTextLayoutMarshaller::SetText(const FString& SourceString, FTextLayout& TargetTextLayout)
{
	if (bSyntaxHighlightingEnabled)
	{
		TArray<FPrologSyntaxTokenizer::FTokenizedLine> TokenizedLines;
...
bool FPLRichTextSyntaxHighlighterTextLayoutMarshaller::RequiresLiveUpdate() const
{
	return bSyntaxHighlightingEnabled;
}

void FPLRichTextSyntaxHighlighterTextLayoutMarshaller::EnableSyntaxHighlighting(const bool bEnable)
{
	bSyntaxHighlightingEnabled = bEnable;
	MakeDirty();
}

bool FPLRichTextSyntaxHighlighterTextLayoutMarshaller::IsSyntaxHighlightingEnabled() const
{
	return bSyntaxHighlightingEnabled;
}
...
TSharedRef< FPLRichTextSyntaxHighlighterTextLayoutMarshaller > FPLRichTextSyntaxHighlighterTextLayoutMarshaller::Create(const FSyntaxTextStyle& InSyntaxTextStyle)
{
	TArray<FPrologSyntaxTokenizer::FRule> TokenizerRules;

	// operators
	for (const auto& Operator : Operators)
	{
		TokenizerRules.Emplace(FPrologSyntaxTokenizer::FRule(Operator, FPrologSyntaxTokenizer::ESyntaxType::Operators));
	}
	// Terminals
	for (const auto& Terminal : Terminals)
	{
		TokenizerRules.Emplace(FPrologSyntaxTokenizer::FRule(Terminal, FPrologSyntaxTokenizer::ESyntaxType::Terminals));
	}

	// keywords
	for (const auto& Keyword : Keywords)
	{
		TokenizerRules.Emplace(FPrologSyntaxTokenizer::FRule(Keyword, FPrologSyntaxTokenizer::ESyntaxType::Keywords));
	}

	// Pre-processor Keywords
	for (const auto& BuiltInKeyword : BuiltInKeywords)
	{
		TokenizerRules.Emplace(FPrologSyntaxTokenizer::FRule(BuiltInKeyword, FPrologSyntaxTokenizer::ESyntaxType::Function));
	}
	return MakeShareable(new FPLRichTextSyntaxHighlighterTextLayoutMarshaller(FPrologSyntaxTokenizer::Create(TokenizerRules), InSyntaxTextStyle));
}



void FPLRichTextSyntaxHighlighterTextLayoutMarshaller::ParseTokens(const FString& SourceString, FTextLayout& TargetTextLayout, TArray<FPrologSyntaxTokenizer::FTokenizedLine> TokenizedLines)
{
	...
	for (const FPrologSyntaxTokenizer::FTokenizedLine& TokenizedLine : TokenizedLines)
	{
		...
		for (const FPrologSyntaxTokenizer::FToken& Token : TokenizedLine.Tokens)
		{
			...
			if (!bIsWhitespace)
			{
				...
				if (Token.Type == FPrologSyntaxTokenizer::ETokenType::Syntax)
				{
					...
					else if (ParseState == EParseState::None && !hasNextChar && Token.SyntaxType == FPrologSyntaxTokenizer::ESyntaxType::Keywords)
					{
						RunInfo.Name = TEXT("SyntaxHighlight.PY.Keyword");
						TextBlockStyle = SyntaxTextStyle.KeywordTextStyle;
						ParseState = EParseState::None;
					}
					else if (ParseState == EParseState::None && !hasNextChar && Token.SyntaxType == FPrologSyntaxTokenizer::ESyntaxType::Function)
					{
					...
					else if (ParseState == EParseState::LookingForDefine && Token.SyntaxType == FPrologSyntaxTokenizer::ESyntaxType::Terminals)
					{
						RunInfo.Name = TEXT("SyntaxHighlight.PY.Normal");
						TextBlockStyle = SyntaxTextStyle.NormalTextStyle;
						ParseState = EParseState::None;
					}
					else if (ParseState == EParseState::None && Token.SyntaxType == FPrologSyntaxTokenizer::ESyntaxType::Operators)
					{
				...
				if (Token.Type == FPrologSyntaxTokenizer::ETokenType::Literal || !bHasMatchedSyntax)
				{
