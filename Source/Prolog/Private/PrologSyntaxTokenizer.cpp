...
TSharedRef< FPrologSyntaxTokenizer > FPrologSyntaxTokenizer::Create(TArray<FRule> InRules)
{
	return MakeShareable(new FPrologSyntaxTokenizer(MoveTemp(InRules)));
}

FPrologSyntaxTokenizer::~FPrologSyntaxTokenizer()
{
}

void FPrologSyntaxTokenizer::Process(TArray<FTokenizedLine>& OutTokenizedLines, const FString& Input)
{
#if UE_ENABLE_ICU
...
FPrologSyntaxTokenizer::FPrologSyntaxTokenizer(TArray<FRule> InRules)
	: Rules(MoveTemp(InRules))
{
}

void FPrologSyntaxTokenizer::TokenizeLineRanges(const FString& Input, const TArray<FTextRange>& LineRanges, TArray<FTokenizedLine>& OutTokenizedLines)
{
