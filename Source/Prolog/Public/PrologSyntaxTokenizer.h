...
public:
	 ...
	 */
	static TSharedRef< FPrologSyntaxTokenizer > Create(TArray<FRule> InRules);

	virtual ~FPrologSyntaxTokenizer();

	void Process(TArray<FTokenizedLine>& OutTokenizedLines, const FString& Input);

private:

	FPrologSyntaxTokenizer(TArray<FRule> InRules);
